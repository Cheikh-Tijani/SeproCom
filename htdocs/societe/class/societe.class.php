...
class Societe extends CommonObject
{
    ...
    function create($user)
    {
        ...
        if ($result >= 0)
        {
            $sql = "INSERT INTO ".MAIN_DB_PREFIX."societe (nom, name_alias, entity, datec, fk_user_creat, canvas, status, ref_int, ref_ext, fk_stcomm, fk_incoterms, location_incoterms ,import_key, fk_multicurrency,typepiece, numeropiece, multicurrency_code)";
            ...
            $sql.= ", '".$this->db->escape($this->typepiece)."'";
            $sql.= ", '".$this->db->escape($this->numeropiece)."'";
    ...
    function update($id, $user='', $call_trigger=1, $allowmodcodeclient=0, $allowmodcodefournisseur=0, $action='update', $nosyncmember=1)
    {
        ...
        // Nouveau Champs
        $this->typepiece		= trim($this->typepiece);
        $this->numeropiece		= trim($this->numeropiece);
        // End Nouveau Champs
        ...
        if ($result >= 0)
        {
            ...
            //Update Nouveau champs
            $sql .= ",typepiece = '". $this->db->escape($this->typepiece) ."'";
            $sql .= ",numeropiece = '". $this->db->escape($this->numeropiece) ."'";
            //End
    ...
    function fetch($rowid, $ref='', $ref_ext='', $ref_int='', $idprof1='',$idprof2='',$idprof3='',$idprof4='',$idprof5='',$idprof6='')
    {
        ...
        $sql .= ', s.status, s.typepiece, s.numeropiece';
        $sql .= ', s.price_level';
        ...
        if ($resql)
        {
            ...
            elseif ($num)   // $num = 1
            {
               ...
               // var_dump($this->typepiece, $this->numeropiece);die;
                ...
                //Recup nouveau champs
                $this->typepiece = $obj->typepiece;
                $this->numeropiece = $obj->numeropiece;
                //End Recup nouveau champs
