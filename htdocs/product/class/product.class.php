...
class Product extends CommonObject
{
	...
	var $niveauetage;
    /**
	 * Product numeroappartement
	 * @var string
	 */
	var $numeroappartement;
    /**
	 * Product tom
	 * @var string
	 */
	...
	var $ascenseur;
    /**
	 * Product groupe electrogene
	 * @var string
	 */
	var $groupeelectrogene;
    /**
     * Product descripion
     * @var string
     */
	...
	function create($user,$notrigger=0)
	{
        ...
        if(empty($this->proprietaire)){
            $referenceseq = $this->ref;
        }elseif (!empty($this->proprietaire)){
            $referenceseq = $this->genererlareference($this->ref,$this->proprietaire);
        }

		$this->ref = dol_string_nospecial(trim($referenceseq));
        //Nouveau champs ajoute CTN
		$this->numeroappartement = trim($this->numeroappartement);
		$this->orientation = trim($this->orientation);
		$this->niveauetage = trim($this->niveauetage);
		$this->zone = trim($this->zone);
		$this->groupeelectrogene = trim($this->groupeelectrogene);
		$this->ascenseur = trim($this->ascenseur);
        $this->proprietaire = trim($this->proprietaire);
        $this->typeproduit = trim($this->typeproduit);
        $this->position = trim($this->position);
		$this->tom = trim($this->tom);
		$this->tlv = trim($this->tlv);
        //Fin nouveau champs ajoute CTN
		$this->label = trim($this->label);
		$this->price_ttc=price2num($this->price_ttc);
		$this->price=price2num($this->price);
		$this->price_min_ttc=price2num($this->price_min_ttc);
		$this->price_min=price2num($this->price_min);
        if(empty($this->proprietaire)) $this->proprietaire = 1;
        if(empty($this->tlv))   $this->tlv=0;
        if(empty($this->tom))   $this->tom=0;
        ...
        if ($result >= 0)
        {
			...
			if ($result)
			{
				...
				if ($obj->nb == 0)
				{
					...
					$sql.= ", tom";
					$sql.= ", ascenseur";
					$sql.= ", groupeelectrogene";
					$sql.= ", zone";
					$sql.= ", orientation";
					$sql.= ", numeroappartement";
					$sql.= ", niveauetage";
					$sql.= ", proprietaire";
					$sql.= ", typeproduit";
					$sql.= ", position";
                    //Fin Nouveau champs CTN
					$sql.= ", ref_ext";
					$sql.= ", price_min";
					$sql.= ", price_min_ttc";
					$sql.= ", label";
                    ...
                    //Nouveau champs CTN
					$sql.= ", '".$this->db->escape($this->tlv)."'";
					$sql.= ", '".$this->db->escape($this->tom)."'";
					$sql.= ", '".$this->db->escape($this->ascenseur)."'";
					$sql.= ", '".$this->db->escape($this->groupeelectrogene)."'";
					$sql.= ", '".$this->db->escape($this->zone)."'";
					$sql.= ", '".$this->db->escape($this->orientation)."'";
					$sql.= ", '".$this->db->escape($this->numeroappartement)."'";
					$sql.= ", '".$this->db->escape($this->niveauetage)."'";
					$sql.= ", '".$this->db->escape($this->proprietaire)."'";
					$sql.= ", '".$this->db->escape($this->typeproduit)."'";
	...
	function update($id, $user, $notrigger=false, $action='update')
	{
        ...
        if ($result >= 0)
        {
            ...
            //Nouveau champs CTN
			$sql.= ", zone = '" . $this->db->escape($this->zone) ."'";
			$sql.= ", orientation = '" . $this->db->escape($this->orientation) ."'";
			$sql.= ", numeroappartement = '" . $this->db->escape($this->numeroappartement) ."'";
			$sql.= ", niveauetage = '" . $this->db->escape($this->niveauetage) ."'";
			$sql.= ", tom = '" . $this->db->escape($this->tom) ."'";
			$sql.= ", tlv = '" . $this->db->escape($this->tlv) ."'";
			$sql.= ", ascenseur = '" . $this->db->escape($this->ascenseur) ."'";
			$sql.= ", groupeelectrogene = '" . $this->db->escape($this->groupeelectrogene) ."'";
			$sql.= ", proprietaire = '" . $this->db->escape($this->proprietaire) ."'";
			$sql.= ", typeproduit = '" . $this->db->escape($this->typeproduit) ."'";
	...
	function fetch($id='', $ref='', $ref_ext='', $ignore_expression=0)
	{
		...
		$sql.= " fk_price_expression, price_autogen, proprietaire,typeproduit, position, zone, tom, niveauetage, numeroappartement, orientation, tlv, ascenseur, groupeelectrogene";
		$sql.= " FROM ".MAIN_DB_PREFIX."product";
		...
		if ( $resql )
		{
			if ($this->db->num_rows($resql) > 0)
			{
                ...
                $this->numeroappartement        = $obj->numeroappartement;
                $this->niveauetage              = $obj->niveauetage;
                $this->tom                      = $obj->tom;
                $this->tlv                      = $obj->tlv;
                $this->ascenseur                = $obj->ascenseur;
                $this->groupeelectrogene        = $obj->groupeelectrogene;
                $this->proprietaire             = $obj->proprietaire;
                $this->typeproduit              = $obj->typeproduit;
    ...
    function genererlareference($ref, $proprietaire)
    {
        $sql = "SELECT nom FROM ".MAIN_DB_PREFIX."societe WHERE rowid = ".$proprietaire;
        ...
        return substr($ref, 0, 9) . strtoupper(substr($nom, 0,2))."-" . substr($ref, 10);
    }
}
