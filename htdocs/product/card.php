	...
	*/
    ...
    if ($action == 'add' && ($user->rights->produit->creer || $user->rights->service->creer))
    {
        ...
        if (! $error)
        {
	        ...
	        $object->orientation             = GETPOST('orientation');
	        $object->niveauetage             = GETPOST('niveauetage');
	        $object->zone                    = GETPOST('zone');
	        $object->tom                     = GETPOST('tom');
	        $object->tlv                     = GETPOST('tlv');
	        $object->ascenseur               = GETPOST('ascenseur');
	        $object->groupeelectrogene       = GETPOST('groupeelectrogene');
	        $object->numeroappartement       = GETPOST('numeroappartement');
	        $object->proprietaire            = GETPOST('proprietaire');
	        $object->position                = GETPOST('position');
	        $object->typeproduit             = GETPOST('typeproduit');

	        $object->units                   = GETPOST('units');
...
else
{
    ...
    if ($action == 'create' && ($user->rights->produit->creer || $user->rights->service->creer))
    {
        ...
        $initialuser = strtoupper(substr($user->getFullName($langs), 0,2));
        print '<td class="fieldrequired" width="20%">'.$langs->trans("Ref").'</td><td colspan="3"><input name="ref" size="32" maxlength="128" value="'.dol_escape_htmltag(GETPOST('ref')?GETPOST('ref'):$initialuser.$tmpcode).'">';
        if ($refalreadyexists)
        {
        ...
        //Proprietaire ou Fournisseur
        print '<tr><td>'.$langs->trans("Proprietaire / Frs").'</td><td colspan="3">';
        print $form->select_proprietaire(GETPOST('proprietaire','int'),'proprietaire');
        print '</td></tr>';

        //Type de produit
        print '<tr><td>'.$langs->trans("Type de Produit").'</td><td colspan="3">';
        print $form->select_typedeproduit(GETPOST('typeproduit','int'),'typeproduit');
        print '</td></tr>';

        //Position
        print '<tr><td>'.$langs->trans("Position").'</td><td colspan="3">';
        $statutarray = array('NULL' => '', 'Gauche' => 'Gauche', 'Droite' => 'Droite');
        print $form->selectarray('position',$statutarray,GETPOST('position'));
        print '</td></tr>';
        ...
        // Public Zone
        print '<tr><td>'.$langs->trans("Zone").'</td><td>';
        print $form->select_zone(GETPOST('zone','int'),'zone');
        print '</td><td>'.$langs->trans("Niveau Etage").'</td><td>';
        print $form->select_niveauetage(GETPOST('niveauetage','int'),'niveauetage');
        print '</td></tr>';

        // Public Niveau Etage
        print '<tr><td>'.$langs->trans("Orientation").'</td><td>';
        print $form->select_orientation(GETPOST('orientation','int'),'orientation');
        print '</td><td>'.$langs->trans("Numero Appartement").'</td><td>';
		print '<input type="text" name="numeroappartement" value="'.GETPOST('numeroappartement').'">';
        print '</td></tr>';
        ...
        else
		{
            ...
            print '</td><td>'.$langs->trans("TOM").'</td><td>';
            print $form->select_tom(GETPOST('tom','int'),'tom');
           // print $form->selectarray('tlv',$statutarray,GETPOST('tom'));
            print '</td></tr>';

            // Public TLV et Autre support
            print '<tr><td>'.$langs->trans("TLV").'</td><td>';
            print $form->select_tom(GETPOST('tlv','int'),'tlv');
            print '</td><td>'.$langs->trans("Ascenseur").'&nbsp&nbsp';
            $statutarray = array('' => '', 'Oui' => 'Oui', 'Non' => 'Non');
            print $form->selectarray('ascenseur',$statutarray,GETPOST('ascenseur'));
            print '</td><td>'.$langs->trans("Groupe Electrogene").'&nbsp&nbsp';
            $statutarray = array('' => '', 'Oui' => 'Oui', 'Non' => 'Non');
            print $form->selectarray('groupeelectrogene',$statutarray,GETPOST('groupeelectrogene'));
            print '</td></tr>';
    ...
    else if ($object->id > 0)
    {
        ...
        if ($action == 'edit' &&  ((($object->type == Product::TYPE_PRODUCT && $user->rights->produit->creer) ||  ($object->type == Product::TYPE_SERVICE && $user->rights->service->creer))))
        {
            ...
            //Proprietaire ou Fournisseur
            print '<tr><td>'.$langs->trans("Proprietaire / Frs").'</td><td colspan="3">';
           // $statutarray = array('1' => 'Selectionnez');
           // print $form->selectarray('proprietaire',$statutarray,$object->proprietaire);
            print $form->select_proprietaire($object->proprietaire, 'proprietaire', '', 0, 'minwidth100 maxwidthonsmartphone');
            print '</td></tr>';

            //Type de produit
            print '<tr><td>'.$langs->trans("Type de produit").'</td><td colspan="3">';
            //$statutarray = array('NULL' => 'Selectionnez');
            //print $form->selectarray('typeproduit',$statutarray,$object->typeproduit);
            print $form->select_typedeproduit($object->typeproduit, 'typeproduit', '', 0, 'minwidth100 maxwidthonsmartphone');
            print '</td></tr>';

            //Position
            print '<tr><td>'.$langs->trans("Position").'</td><td colspan="3">';
            $statutarray = array('NULL' => '', 'Gauche' => 'Gauche', 'Droite' => 'Droite');
            print $form->selectarray('position',$statutarray,$object->position);
            print '</td></tr>';
            ...
            // Public Zone
            print '<tr><td>'.$langs->trans("Zone").'</td><td>';
            print $form->select_zone($object->zone, 'zone', '', 0, 'minwidth100 maxwidthonsmartphone');
            print '</td><td>'.$langs->trans("Niveau Etage").'</td><td>';
            print $form->select_niveauetage($object->niveauetage, 'niveauetage', '', 0, 'minwidth100 maxwidthonsmartphone');
            print '</td></tr>';

            // Public Niveau Etage
            print '<tr><td>'.$langs->trans("Orientation").'</td><td>';
            print $form->select_orientation($object->orientation, 'orientation', '', 0, 'minwidth100 maxwidthonsmartphone');
            print '</td><td>'.$langs->trans("Numero Appartement").'</td><td>';
            print '<input type="text" name="numeroappartement" value="'.$object->numeroappartement.'">';
            print '</td></tr>';
            ...
            print $form->load_tva("tva_tx",-1,$mysoc,'');
            print '</td><td>'.$langs->trans("TOM").'</td><td>';
            print $form->select_tom($object->tom, 'tom', '', 0, 'minwidth100 maxwidthonsmartphone');
            print '</td></tr>';

            // Public TLV et Autre support
            print '<tr><td>'.$langs->trans("TLV").'</td><td>';
            print $form->select_tlv($object->tlv, 'tlv', '', 0, 'minwidth100 maxwidthonsmartphone');
            print '</td><td>'.$langs->trans("Ascenseur").'&nbsp&nbsp';
            $statutarray = array('' => '', 'Oui' => 'Oui', 'Non' => 'Non');
            print $form->selectarray('ascenseur',$statutarray,$object->ascenseur);
            print '</td><td>'.$langs->trans("Groupe Electrogene").'&nbsp&nbsp';
            $statutarray = array('' => '', 'Oui' => 'Oui', 'Non' => 'Non');
            print $form->selectarray('groupeelectrogene',$statutarray,$object->groupeelectrogene);
            print '</td></tr>';
        ...
        else
		{
            ...
            */
            ...
            // Public Zone
            print '<tr><td class="titlefield">'.$langs->trans("Zone").'</td><td colspan="2">';
                  // .$object->zone;
            print $form->convertRowIdtoLibelle($object->zone, 'libelle', 'c_zone');
            print  '</td></tr>';

            // Public Orientation
            print '<tr><td class="titlefield">'.$langs->trans("Orientation").'</td><td colspan="2">';
            print $form->convertRowIdtoLibelle($object->orientation, 'libelle', 'c_orientation').'</td></tr>';

            // Public Niveau Etage
            print '<tr><td class="titlefield">'.$langs->trans("Niveau Etage").'</td><td colspan="2">';
            print $form->convertRowIdtoLibelle($object->niveauetage, 'libelle', 'c_etage').'</td></tr>';

            // Public Numero Appartement
            print '<tr><td class="titlefield">'.$langs->trans("Numero Appartement").'</td><td colspan="2">'
                .$object->numeroappartement.'</td></tr>';

            //Proprietaire ou Fournisseur
            print '<tr><td class="titlefield">'.$langs->trans("Proprietaire / Frs").'</td><td colspan="3">';
            //print $object->proprietaire;
            //var_dump($form->convertRowIdtoLibelle($object->proprietaire, 'nom', 'societe'));die;
            print $form->convertRowIdtoLibelle($object->proprietaire, 'nom', 'societe');
            print '</td></tr>';

            //Type de produit
            print '<tr><td class="titlefield">'.$langs->trans("Type de produit").'</td><td colspan="3">';
            print $form->convertRowIdtoLibelle($object->typeproduit, 'libelle', 'c_typeproduit');
            print '</td></tr>';

            //Position
            print '<tr><td class="titlefield">'.$langs->trans("Position").'</td><td colspan="3">';
