...
class Form
{
     ...
     *  @return		void
     */
    function select_produits($selected='', $htmlname='productid', $filtertype='', $limit=20, $price_level=0, $status=1, $finished=2, $selected_input_value='', $hidelabel=0, $ajaxoptions=array(), $socid=0, $idpropal=0)
    {
        global $langs,$conf;
        ...
        else
		{
            ...
            if(!empty($idpropal)){
             ...
             print $this->select_produits_list_by_zone($selected,$htmlname,$filtertype,$limit,$price_level,'',$status,$finished,0,$socid,$idpropal);
            }else{
    ...
    //Liste des produits en fonction de la zone
    /**
     *	Return list of products for a customer
     *
     *	@param      int		$selected       Preselected product
     ...
     *  @return     array    				Array of keys for json
     */
    function select_produits_list_by_zone($selected='',$htmlname='productid',$filtertype='',$limit=20,$price_level=0,$filterkey='',$status=1,$finished=2,$outputmode=0,$socid=0, $idpropal=0)
    {
        global $langs,$conf,$user,$db;
        ...
        //Code pour la prospection en fonction de la zone
        $sqlzone = "select zone1, zone2, zone3, zone4, budgetminimal, budgetmaximal, typeproduit from ".MAIN_DB_PREFIX."propal where rowid = ".$idpropal;
        //var_dump($sqlzone);die;
        $resultzone = $this->db->query($sqlzone);
        if ($resultzone){
            $objp = $this->db->fetch_object($resultzone);
            $zone1 = $objp->zone1;
            $zone2 = $objp->zone2;
            $zone3 = $objp->zone3;
            $zone4 = $objp->zone4;
            $budgetminimal = $objp->budgetminimal;
            $budgetmaximal = $objp->budgetmaximal;
        }
        //var_dump($zone1, $zone2, $zone3, $zone4);die;
        $sql = "SELECT ";
        $sql.= " p.rowid, p.label, p.ref, p.description, p.barcode, p.fk_product_type, p.price, p.price_ttc, p.price_base_type, p.tva_tx, p.duration, p.stock, p.fk_price_expression";
        ...
        //Critere par zone
        if(!empty($zone1)){
            $sql.= " AND p.zone Like '".$zone1."'";
        }
        if(!empty($zone2)){
            $sql.= " OR p.zone Like '".$zone2."'";
        }
        if(!empty($zone3)){
            $sql.= " OR p.zone Like '".$zone3."'";
        }
        if(!empty($zone4)){
            $sql.= " OR p.zone Like '".$zone4."'";
        }
        ...
        // Build output string
        dol_syslog(get_class($this)."::select_produits_list search product", LOG_DEBUG);//var_dump($sql);die;
        $result=$this->db->query($sql);
        if ($result)
        {
            require_once DOL_DOCUMENT_ROOT.'/product/class/product.class.php';
    ...
    //Ajout de nouvelle focntion CTN

    //TOM

    function select_tom($selected='',$htmlname='tom',$htmloption='',$maxlength=0,$morecss='minwidth300')
    {
        global $conf,$langs;
        ...
        $sql = "SELECT rowid, code, taux FROM ".MAIN_DB_PREFIX."c_tom";
        $sql.= " WHERE active > 0";
       ...
       // dol_syslog(get_class($this)."::select_country", LOG_DEBUG);
        ...
        if ($resql)
        {
            $out.= '<select id="select'.$htmlname.'" class="selecttom'.($morecss?' '.$morecss:'').'" name="'.$htmlname.'" '.$htmloption.'>';
            $num = $this->db->num_rows($resql);
            $i = 0;
            if ($num)
            {
                ...
                {
                    $obj = $this->db->fetch_object($resql);
                    $tomArray[$i]['rowid'] 		= $obj->rowid;
                    $tomArray[$i]['code'] 	    = $obj->code;
                    $tomArray[$i]['taux'] 	    = $obj->taux;
                    $i++;
                }

                sort($tomArray);

                foreach ($tomArray as $row)
                {
                    if ($selected && $selected != '-1' && ($selected == $row['rowid'] || $selected == $row['taux']) )
                    {
                        $foundselected=true;
                        $out.= '<option value="'.$row['rowid'].'" selected>';
                    }
                    else
                    {
                        ...
                        $out.= dol_trunc($row['taux'],$maxlength,'middle')."%";
                        $out.= '</option>';
                }
            }
            $out.= '</select>';
    ...
    //TLV
    function select_tlv($selected='',$htmlname='tlv',$htmloption='',$maxlength=0,$morecss='minwidth300')
    {
        global $conf,$langs;
        ...
        $sql = "SELECT rowid, code, taux FROM ".MAIN_DB_PREFIX."c_tlv";
        $sql.= " WHERE active > 0";
       ...
       // dol_syslog(get_class($this)."::select_country", LOG_DEBUG);
        ...
        if ($resql)
        {
            $out.= '<select id="select'.$htmlname.'" class="selecttlv'.($morecss?' '.$morecss:'').'" name="'.$htmlname.'" '.$htmloption.'>';
            $num = $this->db->num_rows($resql);
            $i = 0;
            if ($num)
            {
    ...
    //TLV
    function select_proprietaire($selected='',$htmlname='proprietaire',$htmloption='',$maxlength=0,$morecss='minwidth300')
    {
        global $conf,$langs;
        ...
        $sql = "SELECT rowid, nom, fournisseur FROM ".MAIN_DB_PREFIX."societe";
        ...
        if ($resql)
        {
            ...
            if ($num)
            {
                ...
                while ($i < $num)
                {
                    $obj = $this->db->fetch_object($resql);
                    $tomArray[$i]['rowid'] 		 = $obj->rowid;
                    $tomArray[$i]['nom'] 	     = $obj->nom;
                    $tomArray[$i]['fournisseur'] = $obj->fournisseur;
                    $i++;
                }
                ...
                foreach ($tomArray as $row)
                {
                    if($row['fournisseur'] == 1)
                    {
                        if ($selected && $selected != '-1' && ($selected == $row['rowid'] || $selected == $row['nom']) )
                        {
                            $foundselected=true;
                            $out.= '<option value="'.$row['rowid'].'" selected>';
                        }
                        ...
                        $out.= dol_trunc($row['nom'],$maxlength,'middle');
                        $out.= '</option>';
                    }
    ...
    //Zone
    function select_zone($selected='',$htmlname='zone',$htmloption='',$maxlength=0,$morecss='minwidth300')
    {
        global $conf,$langs;
        ...
        $sql = "SELECT rowid, code, libelle FROM ".MAIN_DB_PREFIX."c_zone";
        $sql.= " WHERE active > 0";
        ...
        if ($resql)
        {
            ...
            if ($num)
            {
                ...
                while ($i < $num)
                {
                    ...
                    $tomArray[$i]['rowid'] 		 = $obj->rowid;
                    $tomArray[$i]['code'] 	     = $obj->code;
                    $tomArray[$i]['libelle']     = $obj->libelle;
                    $i++;
                }
                ...
                foreach ($tomArray as $row)
                {
                    if ($selected && $selected != '-1' && ($selected == $row['rowid'] || $selected == $row['libelle']) )
                    {
                        $foundselected=true;
                        $out.= '<option value="'.$row['rowid'].'" selected>';
                    }
    ...
    //Etage
    function select_niveauetage($selected='',$htmlname='niveauetage',$htmloption='',$maxlength=0,$morecss='minwidth300')
    {
        global $conf,$langs;
        ...
        $sql = "SELECT rowid, code, libelle FROM ".MAIN_DB_PREFIX."c_etage";
        $sql.= " WHERE active > 0";
    ...
    //Orientation
    function select_orientation($selected='',$htmlname='orientation',$htmloption='',$maxlength=0,$morecss='minwidth300')
    {
        global $conf,$langs;
        ...
        $sql = "SELECT rowid, code, libelle FROM ".MAIN_DB_PREFIX."c_orientation";
        $sql.= " WHERE active > 0";
    ...
    //Type de produit
    function select_typedeproduit($selected='',$htmlname='typeproduit',$htmloption='',$maxlength=0,$morecss='minwidth300')
    {
        global $conf,$langs;
        ...
        $sql = "SELECT rowid, code, libelle FROM ".MAIN_DB_PREFIX."c_typeproduit";
        $sql.= " WHERE active > 0";
    ...
    //convertir les rowId en libelle correspondant
    function convertRowIdtoLibelle($id,$colone ,$nametable)
    {
        if (empty($id))
            return '';

        $sql = "SELECT ".$colone." as libelle FROM ".MAIN_DB_PREFIX.$nametable." WHERE rowid = ".$id;
        if($nametable != 'societe')
        $sql.= " AND active > 0";
        //var_dump($sql);
        $resql=$this->db->query($sql);
        if ($resql)
